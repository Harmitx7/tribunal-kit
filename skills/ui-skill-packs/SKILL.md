...
Before generating, refactoring, or reviewing code in the `ui-skill-packs` domain, inspect these 5 critical parameters:
...
- **Activate when:** Use when Mandatory skill loading packs for UI generation. Consolidates 42+ individual UI skills into 3 tiered packs (Core Craft, Immersive & System, High-End Design) to guarantee non-generic, high-fidelity UI outputs.
- **DO NOT activate when:** The task falls outside the `ui-skill-packs` domain or is managed by a different dedicated specialist.
