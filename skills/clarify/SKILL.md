...
Before generating, refactoring, or reviewing code in the `clarify` domain, inspect these 5 critical parameters:
...
- **Activate when:** Use when Improve UX microcopy, label optimization, error messages, and cognitive clarity. Use when text in a UI is confusing, wordy, ambiguous, or unhelpful.
- **DO NOT activate when:** The task falls outside the `clarify` domain or is managed by a different dedicated specialist.
