...
Before generating, refactoring, or reviewing code in the `distill` domain, inspect these 5 critical parameters:
...
- **Activate when:** Use when Simplify noisy interfaces by removing non-essential visual and operational complexity. Use when a UI has too many options, crowded toolbars, redundant text, or unnecessary visual containers.
- **DO NOT activate when:** The task falls outside the `distill` domain or is managed by a different dedicated specialist.
