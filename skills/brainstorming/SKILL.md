...
Before generating, refactoring, or reviewing code in the `brainstorming` domain, inspect these 5 critical parameters:
...
- **Activate when:** Use when Socratic methodology, ideation, and architectural exploration mastery. Generating extensive feature options, analyzing trade-offs, questioning assumptions, mind-mapping components, and delaying execution. Use when evaluating new features, defining project goals, or guiding users through ambiguous design spaces.
- **DO NOT activate when:** The task falls outside the `brainstorming` domain or is managed by a different dedicated specialist.
