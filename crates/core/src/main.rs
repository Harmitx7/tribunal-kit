...
async fn cmd_sync(path: &str, quiet: bool) -> Result<()> {
    ...
    let bridge_files = vec![
        ...
        ("AGENTS.md", target.join("AGENTS.md")),
    ];
...
async fn sync_ide_bridges(target: &PathBuf, agent_dir: &PathBuf) -> Result<()> {
    ...
    let codex_rules = format!(
        "# Tribunal Kit — Codex Bridge\n# Auto-generated by tribunal-kit sync.\n# Source: .agent/rules/GEMINI.md\n\n{}",
        rules_content
    );

    // Fire ALL bridge writes concurrently
    tokio::join!(
        ...
        force_write_bridge(target.join(".claude").join("CLAUDE.md"), claude_rules.clone()),
        force_write_bridge(target.join("CLAUDE.md"), claude_rules),
        force_write_bridge(target.join("AGENTS.md"), codex_rules),
    );
...
async fn generate_ide_bridges(target: &PathBuf, agent_dest: &PathBuf) -> Result<()> {
    ...
    let codex_rules = format!(
        "# Tribunal Kit \u{2014} Codex Bridge\n# Auto-generated by tribunal-kit init.\n# Source: .agent/rules/GEMINI.md\n\n{}",
        rules_content
    );

    // Fire ALL bridge writes concurrently via tokio::join!
    tokio::join!(
        ...
        write_bridge(target.join(".claude").join("CLAUDE.md"), claude_rules.clone()),
        write_bridge(target.join("CLAUDE.md"), claude_rules),
        write_bridge(target.join("AGENTS.md"), codex_rules),
    );
