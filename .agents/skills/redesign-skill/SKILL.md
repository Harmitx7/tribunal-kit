...
Before generating, refactoring, or reviewing code in the `redesign-skill` domain, inspect these 5 critical parameters:
...
- **Activate when:** Use when Audit and upgrade existing interfaces to premium visual quality while preserving product functionality and business logic.
- **DO NOT activate when:** The task falls outside the `redesign-skill` domain or is managed by a different dedicated specialist.
