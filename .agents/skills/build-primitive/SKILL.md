...
Before generating, refactoring, or reviewing code in the `build-primitive` domain, inspect these 5 critical parameters:
...
- **Activate when:** Use when Build foundational, unstyled, accessible UI primitives from scratch with strong ARIA attributes, keyboard navigation, focus traps, and state management. Use when creating custom Headless UI components (Dialog, Combobox, Accordion, Popover, Menu).
- **DO NOT activate when:** The task falls outside the `build-primitive` domain or is managed by a different dedicated specialist.
