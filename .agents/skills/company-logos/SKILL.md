...
Before generating, refactoring, or reviewing code in the `company-logos` domain, inspect these 5 critical parameters:
...
- **Activate when:** Use when Social proof rows, logo grids, customer carousels, and trust badges layout rules for balanced visual weight and responsive alignment.
- **DO NOT activate when:** The task falls outside the `company-logos` domain or is managed by a different dedicated specialist.
