...
Before generating, refactoring, or reviewing code in the `quieter` domain, inspect these 5 critical parameters:
...
- **Activate when:** Use when Tone down overly loud, noisy, visually aggressive, or distracting designs while maintaining high visual quality. Use when a UI feels cluttered, overwhelming, tacky, or visually hyperactive.
- **DO NOT activate when:** The task falls outside the `quieter` domain or is managed by a different dedicated specialist.
