...
Before generating, refactoring, or reviewing code in the `ui-reasoning-engine` domain, inspect these 5 critical parameters:
...
- **Activate when:** Use when Enforces a structured cognitive pipeline that forces the agent to analyze user goals, information architecture, platform constraints, and design tokens before styling any component.
- **DO NOT activate when:** The task falls outside the `ui-reasoning-engine` domain or is managed by a different dedicated specialist.
