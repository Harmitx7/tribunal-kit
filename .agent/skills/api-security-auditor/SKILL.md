...
description: Use when API Security auditing mastery. Rate limiting architecture, API key management, payload validation, IDOR (Insecure Direct Object Reference) prevention, mass assignment flaws, GraphQL security, and server-side mitigations. Use when building external APIs, B2B services, or reviewing endpoint security.
version: 5.0.0
...
Before generating, refactoring, or reviewing code in the `api-security-auditor` domain, inspect these 5 critical parameters:
...
- **Activate when:** Use when API Security auditing mastery. Rate limiting architecture, API key management, payload validation, IDOR (Insecure Direct Object Reference) prevention, mass assignment flaws, GraphQL security, and server-side mitigations. Use when building external APIs, B2B services, or reviewing endpoint security.
- **DO NOT activate when:** The task falls outside the `api-security-auditor` domain or is managed by a different dedicated specialist.
