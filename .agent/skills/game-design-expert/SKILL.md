...
- **Activate when:** Operating in tasks requiring Game Design, UX, and Flow State mastery. Replaces fragmented legacy skills. Core gameplay loop design, 3Cs (Character, Camera, Controls), input buffering, coyote time, juice (game feel), telemetry tracking, narrative alignment, and audio spatialization integration. Use when crafting player experience, progression arcs, or systemic balance..
- **DO NOT activate when:** The task falls strictly outside game-design-expert domain or belongs to a different dedicated specialist.
