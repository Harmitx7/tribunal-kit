...
- **Activate when:** Operating in tasks requiring LLM Intent Processing and Gateway Routing mastery. Request classification hierarchies, function routing, confidence scoring, fallback cascades, zero-shot vs few-shot classification patterns, and identifying specialized skills for delegation. Use when parsing raw user input to determine the architectural path of execution..
- **DO NOT activate when:** The task falls strictly outside intelligent-routing domain or belongs to a different dedicated specialist.
