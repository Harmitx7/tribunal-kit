...
- **Activate when:** Operating in tasks requiring Comprehensive Web Application Testing strategy. Test Pyramid, Vitest/Jest for unit logic, React Testing Library for component integrity, MSW (Mock Service Worker) for API layer simulation, and visual regression. Use when setting up testing environments or defining global test strategies across a web stack..
- **DO NOT activate when:** The task falls strictly outside webapp-testing domain or belongs to a different dedicated specialist.
