...
- **Activate when:** Operating in tasks requiring Platform Engineering and Internal Developer Portal (IDP) mastery. Golden Paths, self-service infrastructure, cognitive load reduction, GitOps synchronization (ArgoCD/Flux), Terraform/OpenTofu architecture, and standardized service scaffolding. Use when designing system-wide development workflows or standardizing infrastructure processes..
- **DO NOT activate when:** The task falls strictly outside platform-engineer domain or belongs to a different dedicated specialist.
