...
- **Activate when:** Operating in tasks requiring Performance profiling mastery. Core Web Vitals (LCP, CLS, INP), Lighthouse auditing, JavaScript profiling, React rendering optimization, bundle analysis, memory leak detection, database query profiling (EXPLAIN ANALYZE), load testing, and performance budgets. Use when optimizing performance, debugging slow pages, or establishing performance standards..
- **DO NOT activate when:** The task falls strictly outside performance-profiling domain or belongs to a different dedicated specialist.
