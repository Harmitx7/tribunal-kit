...
Before generating, refactoring, or reviewing code in the `react-doctor` domain, inspect these 5 critical parameters:
...
- **Activate when:** Use when Scan React and Next.js applications for security, performance, re-render inefficiencies, memory leaks, and correctness issues.
- **DO NOT activate when:** The task falls outside the `react-doctor` domain or is managed by a different dedicated specialist.
